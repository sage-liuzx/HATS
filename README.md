...
> 以下命令为示例，请根据仓库实际代码结构、镜像仓库和 K8s 配置进行调整。
...
确保各节点已加入集群，并部署 llama.cpp 推理服务：

```bash
kubectl apply -f deploy/llama-server.yaml
...
```bash
docker build -t <registry>/llama-scheduler:latest -f scheduler/Dockerfile .
docker push <registry>/llama-scheduler:latest
...
```bash
python benchmark/run_tps_benchmark.py --interval 1200
```
...
```bash
python scripts/collect_results.py --output experiments/results
```
...
```text
HATS/
├── scheduler/          # Kubernetes 自定义调度器插件与核心逻辑
├── predictor/          # DistilBERT 输出长度预测器与在线反馈
├── benchmark/          # 动态 TPS 微基准测试
├── deploy/             # Kubernetes YAML 部署文件
├── scripts/            # 任务提交、结果收集、实验脚本
├── experiments/        # 实验结果与图表
├── docs/               # 论文、PPT、架构图、说明文档
│   ├── paper.pdf
│   ├── slides.pptx
│   └── assets/
├── requirements.txt
