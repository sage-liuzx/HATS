...
import (
	...
	"sigs.k8s.io/scheduler-plugins/pkg/hplscore"
	"sigs.k8s.io/scheduler-plugins/pkg/llmadaptivescore"
	"sigs.k8s.io/scheduler-plugins/pkg/multicorescore"
)
...
func main() {
	...
	command := app.NewSchedulerCommand(
		...
		app.WithPlugin(hplscore.Name, hplscore.New),
		app.WithPlugin(llmadaptivescore.Name, llmadaptivescore.New),
		app.WithPlugin(multicorescore.Name, multicorescore.New),
		// ↓↓↓ 新增 ↓↓↓
		//app.WithPlugin(hplscore.Name+".Reserve", hplscore.New),    // Reserve 阶段
		//app.WithPlugin(hplscore.Name+".Bind", hplscore.New),       // Bind 阶段
		//app.WithPlugin(hplscore.Name+".Unreserve", hplscore.New),  // Unreserve 阶段
		app.WithPlugin("PassAllFilter", hplscore.NewPassAllFilter),
	)
